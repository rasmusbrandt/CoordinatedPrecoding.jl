...
function update_MSs!(state::Shi2011_WMMSEState, channel::SinglecarrierChannel,
    ...
    for i = 1:channel.I; for k in served_MS_ids(i, assignment)
        ...
        E = UniformScaling(1.) - state.U[k]'*F
        state.W[k] = inv(Hermitian((E + E')/2))
    end; end
end
...
function update_BSs!(state::Shi2011_WMMSEState, channel::SinglecarrierChannel, 
    ...
    for i in active_BSs(assignment)
        ...
        for k in served_MS_ids(i, assignment)
            state.V[k] = Gamma_eigen.vectors*Diagonal(1./(abs(Gamma_eigen.values) .+ mu_star))*Gamma_eigen.vectors'*channel.H[k,i]'*state.U[k]*state.W[k]
        end
    end
end
...
function optimal_mu(i, Gamma, state::Shi2011_WMMSEState,
    ...
    for k in served_MS_ids(i, assignment)
        bis_M += channel.H[k,i]'*(state.U[k]*(state.W[k]*state.W[k])*state.U[k]')*channel.H[k,i]
    end
    ...
    # mu lower bound
    if maximum(Gamma_eigen_values)/minimum(Gamma_eigen_values) < aux_params["Shi2011_WMMSE:bisection_Gamma_cond"]
        # Gamma is invertible
        mu_lower = 0.
    else
